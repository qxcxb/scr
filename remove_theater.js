...
(async () => {
  ...
  function cleanTheaterPage() {
    ...
    if (monthElem) {
      ...
      if (m) {
        ...
        monthElem.textContent = `01.${pad2(mm)}.${y} — 01.${pad2(
          nextM
        )}.${nextY}`;
  ...
  function scaleDownText() {
    document.querySelectorAll('body, body *').forEach((el) => {
      ...
      if (cs.fontSize && cs.fontSize.endsWith('px')) {
        const num = parseFloat(cs.fontSize);
        el.style.fontSize = num * 0.1 + 'px'; // уменьшаем в 10 раз
      }
    });
  }
  ...
  waitForElement('body > div.scroller > container > div', async () => {
    cleanTheaterPage();
    scaleDownText(); // 👈 уменьшаем текст

    const patchName = (
      document.querySelector(
        'body > div.scroller > container > div > div.blessing_card_area > div > p:nth-child(1)'
      )?.textContent || ''
    ).trim();
    ...
    if (DO_SCREENSHOT) {
      ...
      const target =
        document.querySelector('body > div.scroller > container > div') ||
        document.body;
      ...
      const fileId = await uploadToDrive(
        blob,
        `theater_${patchName || 'unknown'}_${Date.now()}.png`,
        TOKEN
      );
    ...
    // 👇 добавление в таблицу (оставил как у тебя)
    const row = await findRowByPatch(
      SHEETS_ID,
      SHEET_NAME,
      patchName,
      TOKEN
